...
import {
  referentielsActuel,
  positionOptionsActuel,
} from "../../data/referentiels";
import { simulationResultsActuel } from "../../data/results";
...
export default function SimulationParPositionActuel() {
  // Form inputs
  ...
  const currentReferentiel = referentielsActuel[position];
  ...
  const handleLancerSimulation = () => {
    // TODO: This will call backend API with the form data for current process
    // const payload = { position, nombreSacs, nombreDossiers, productivite, type: "actuel" };
    setShowResults(true);
  };

  // Get results only if simulation has been run
  const currentResults = showResults ? simulationResultsActuel[position] : null;

  // Excel export function
  const handleExporterExcel = async () => {
    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Simulation_Position_Actuel");
      ...
      const titleRow = worksheet.addRow([
        "Simulation par Position - Processus Actuel - " + position,
      ]);
      ...
      link.href = url;
      link.download = `Simulation_Position_Actuel_${position.replace(
        /\s+/g,
        "_"
      )}.xlsx`;
    ...
    <div className="max-w-full mx-auto space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={position} onValueChange={setPosition}>
                    ...
                    <SelectContent className="rounded-lg border border-blue-200 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md">
                      {positionOptionsActuel.map((option) => (
                        <SelectItem
                          key={option}
                          value={option}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
          <div className="border-b border-gray-200 px-4 py-1 bg-gradient-to-r from-gray-50 to-blue-50 rounded-t-xl text-center">
            <h3 className="text-base font-bold text-gray-900">
              Référentiel d'activités - Actuel
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
          <div className="border-b border-gray-200 px-4 py-1 bg-gradient-to-r from-gray-50 to-blue-50 rounded-t-xl text-center">
            <h3 className="text-base font-bold text-gray-900">
              Résultats de Simulation - Actuel
            </h3>
          </div>
          <div className="px-4 py-2">
            {/* Results Table */}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-xl font-semibold text-gray-900">
                      Temps unitaire par activité (min) - Processus Actuel
                    </h3>
                  </div>
