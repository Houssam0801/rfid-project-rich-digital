...
import {
  chronogrammeDataActuel,
  positionsActuel,
  chronogrammeParPositionDataActuel,
  chronogrammeTotalActuel,
} from "../../data/chronogrammeData";
import CumulativeGanttChart from "../../components/CumulativeChart";

export default function ChronogrammeTraitementActuel() {
  ...
  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) return chronogrammeDataActuel;
    return chronogrammeDataActuel.filter((item) =>
  ...
  const handleExporterExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Chronogramme_Actuel");
      ...
      chronogrammeDataActuel.forEach((item) => {
      ...
      link.href = url;
      link.download = "Chronogramme_Traitement_Unitaire_Actuel.xlsx";
  ...
  const handleExporterChronogrammeParPosition = async () => {
    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Chronogramme_par_Position_Actuel");
      ...
      Object.entries(chronogrammeParPositionDataActuel).forEach(
        ([position, data]) => {
          const row = worksheet.addRow([
            position,
      ...
      const totalRow = worksheet.addRow([
        "TOTAL GÉNÉRAL",
        chronogrammeTotalActuel.dureeSecondes.toFixed(2),
        chronogrammeTotalActuel.dureeMinutes.toFixed(2),
        chronogrammeTotalActuel.dureeHeures.toFixed(2),
      ]);
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = "Chronogramme_Par_Position_Actuel.xlsx";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
            ...
            <div className="w-full sm:max-w-xs">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  list="positions-list-actuel"
                />
                <datalist id="positions-list-actuel">
                  {positionsActuel.map((pos) => (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
        <div className="border-b border-gray-200 px-4 py-2 bg-gradient-to-r from-gray-50 to-blue-50 rounded-t-xl text-center">
          <h3 className="text-base font-bold text-gray-900">
            Détail des Tâches - Processus Actuel
            {searchTerm && (
            ...
            <div className="p-3 space-y-3">
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <button
                  ...
                  disabled={
                    Object.keys(chronogrammeParPositionDataActuel).length === 0
                  }
              ...
              {viewMode === "table" ? (
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow w-[85%] mx-auto">
                    <div className="rounded-xl border border-gray-200 overflow-hidden">
                      <Table>
                        ...
                        <TableBody>
                          {Object.entries(chronogrammeParPositionDataActuel).map(
                            ([position, data], index) => (
                              <TableRow
                                key={position}
                                className={
                ...
                <div className="flex justify-center">
                  <CumulativeGanttChart
                    data={chronogrammeParPositionDataActuel}
                    total={chronogrammeTotalActuel}
                    unit="Heures"
                  />
                </div>
              )}
