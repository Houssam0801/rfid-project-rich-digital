...
export default function NormesDimensionnement() {
  ...
  const handleExporterNormes = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Normes Actuel");
      ...
      link.href = url;
      link.download = "Normes_de_dimensionnement_Actuel.xlsx";
  ...
  const handleExporterCapacite = async () => {
    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Capacité Nominale Actuel");
      ...
      Object.entries(capaciteNominaleDataActuel).forEach(([key, data]) => {
        const row = worksheet.addRow([
          data.position,
          data.tempsDossier,
          data.dossiersMois || "",
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = "Capacite_Nominale_Actuel.xlsx";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
        ...
        <div className="px-4 py-2">
          <div className="rounded-xl border border-gray-200 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {normesDataActuel.map((item, index) => (
                  <TableRow
                    key={index}
                    className={` text-sm
                     ${
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow w-[90%] mx-auto">
                  <div className="rounded-xl border border-gray-200 overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {Object.entries(capaciteNominaleDataActuel).map(
                          ([key, data], index) => (
                            <TableRow
                              key={key}
                              className={
